...
def minimum_Fs(f):
    ...
    '''
    Fs = 2 * f  # The Nyquist sampling frequency
    return Fs
...
def omega(f, Fs):
    ...
    '''
    omega = 2 * np.pi * (f / Fs)  # Calculate radial frequency
    return omega
...
def pure_tone(omega, N):
    ...
    '''
    n = np.arange(N)  # Create an array [0, 1, 2, ..., N-1]
    x = np.cos(omega * n)  # Generate the pure tone signal
    return x
